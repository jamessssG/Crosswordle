...
document.getElementById('google-signin-btn').onclick = () => {
    auth.signInWithPopup(googleProvider)
        .then(result => {
            ...
            document.getElementById('auth-message').textContent = `Signed in as ${user.displayName}`;
            showLeaderboard();
        })
        .catch(error => {
...
auth.onAuthStateChanged(user => {
    if (user) {
        document.getElementById('auth-message').textContent = `Welcome, ${user.displayName || user.email}`;
        showLeaderboard();
    }
});
